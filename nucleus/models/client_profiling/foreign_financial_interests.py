...
class ForeignFinancialInterests(Base):
    __tablename__ = "foreign_financial_interests"
    ...
    country: Mapped[Optional["CountryLookup"]] = relationship("CountryLookup", back_populates="foreign_financial_interests")
    ...
    nature_of_interest: Mapped[str] = mapped_column(String, nullable=True)

    date_since_held: Mapped[date] = mapped_column(Date, nullable=True)

    total_investment: Mapped[Decimal] = mapped_column(DECIMAL(15, 2), nullable=True)

    income_accrued: Mapped[Decimal] = mapped_column(DECIMAL(15, 2), nullable=True)

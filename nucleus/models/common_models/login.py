...
class Login(Base):
    ...
    __table_args__ = (
        # Prefer NULL over '' for missing emails.
        CheckConstraint(
        ...
        CheckConstraint(
            "(client_id IS NOT NULL) != (advisor_id IS NOT NULL)",
            name="ck_logins_exactly_one_owner",
        ),
    )
    ...
    client_id: Mapped[UUID] = mapped_column(
        SQLUUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=True
    )
    ...
    push_subscriptions: Mapped[list["PushSubscription"]] = relationship(
        "PushSubscription", back_populates="login"
    )
